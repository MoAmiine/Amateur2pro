<x-layouts.app title="Team Details | Amateur2Pro">
    ...
    <div class="max-w-6xl mx-auto px-6 pt-28 pb-20">
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-purple-400 mt-1">
                        Capitaine: <span class="text-white font-semibold">{{ $team->captain?->name ?? 'N/A' }}</span>
                    </p>
                    <p class="text-slate-400 text-sm mt-1">
                        Jeu: <span class="text-purple-400 font-semibold">{{ $team->game->name }}</span>
                    </p>
                    <p class="text-slate-400 text-sm mt-1">
                        {{ $team->users->where('pivot.is_member', true)->count() }} membres
                    </p>
                </div>
                    ...
                    @if (auth()->id() === $team->captain_id)
                        <div class="flex gap-2">
                            ...
                            <form method="POST" action="{{ route('teams.destroy', $team) }}"
                                ...
                                @csrf
                                @method('DELETE')
                                <button class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-xs uppercase font-bold rounded-lg">
                                    Supprimer
                                </button>
                            </form>
                        </div>
        ...
        <div class="mt-6 bg-slate-900/30 border border-white/10 p-6 rounded-xl">
            <h2 class="font-bold mb-2">À propos de l'équipe</h2>
            <p class="text-slate-400">{{ $team->description ?? 'Aucune description disponible.' }}</p>
        </div>

        {{-- Membres --}}
        <div class="mt-6 bg-slate-900/30 border border-white/10 p-6 rounded-xl">
            <h2 class="font-bold mb-4">Membres</h2>
        ...
        {{-- Invite par email — captain seulement --}}
        @auth
            @if (auth()->id() === $team->captain_id)
                <div class="mt-6 bg-slate-900/30 border border-white/10 p-6 rounded-xl">
                    <h2 class="font-bold mb-4">Inviter un joueur par email</h2>
                    <form method="POST" action="{{ route('teams.invite', $team->id) }}" class="flex gap-3">
                        ...
                        <button class="px-6 py-3 bg-purple-600 hover:bg-purple-500 text-white font-bold uppercase text-xs rounded-lg">
                            Inviter
                        </button>
                    </form>
                </div>
        ...
        {{-- Demandes en attente — captain seulement --}}
        @auth
            @if (auth()->id() === $team->captain_id)
                <div class="mt-6 bg-slate-900/30 border border-white/10 p-6 rounded-xl">
                    <h2 class="font-bold mb-4 text-yellow-400">Demandes en attente</h2>
                    @forelse ($team->users->where('pivot.is_member', false) as $user)
                        <div class="flex items-center justify-between bg-slate-800/40 p-3 rounded-lg mb-2">
                            <div>
                                <p class="font-semibold">{{ $user->name }}</p>
                                <p class="text-xs text-yellow-400">En attente d'approbation</p>
                            </div>
                    ...
                    @empty
                        <p class="text-slate-400">Aucune demande en attente.</p>
                    @endforelse
                </div>
            @endif
        @endauth

        {{-- Actions join / leave / pending --}}
        <div class="mt-6 flex gap-3">
            @auth
                @php
                    $member = $team->users->firstWhere('id', auth()->id());
                @endphp
                ...
                @elseif ($member->pivot->is_member == false)
                    {{-- pending --}}
                    <button disabled class="px-5 py-2 bg-yellow-600 text-black rounded-lg cursor-not-allowed">
                        En attente d'approbation
                    </button>

                @elseif ($member->pivot->is_member == true && auth()->id() !== $team->captain_id)
                    {{-- membre confirmé — peut quitter --}}
                    <button disabled class="px-5 py-2 bg-gray-400 text-slate-950 text-xs uppercase font-bold rounded-lg cursor-not-allowed">
                        Membre
                    </button>
