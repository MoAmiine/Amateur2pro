...
<body class="bg-slate-950 text-white min-h-screen p-6 md:p-12">
    ...
    <a href="{{ route('equipes.show', $team) }}" class="absolute top-10 left-10 text-slate-400 hover:text-white transition">← Retour</a>

    <div class="max-w-xl mx-auto mt-16">
        
        <header class="mb-10 text-center">
            <h1 class="text-4xl font-display font-bold italic tracking-tighter mb-2">Modifier TA TEAM</h1>
        ...
        <form action="{{ route('equipes.store') }}" method="POST" class="bg-slate-900/50 p-8 border border-white/10 space-y-6">
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Nom de l'équipe</label>
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Description / Slogan</label>
                <textarea name="description" rows="3" class="w-full bg-slate-950 border border-white/10 p-4 rounded-sm focus:border-purple-500 transition-all outline-none"></textarea>
            </div>
