<x-layouts.app title="Équipes | Amateur2Pro">
    ...
    <div class="max-w-7xl mx-auto px-6 pt-28 pb-20">
        ...
        <form method="GET" class="mb-10 bg-slate-900/30 p-4 border border-white/5 flex flex-wrap gap-4">

            <input type="text" name="search" value="{{ request('search') }}" placeholder="Rechercher une équipe..."
                        ...
                        @auth
                            @if (!$member)
                                <form method="POST" action="{{ route('teams.join', $team) }}" class="flex-1">
                                    @csrf
                                    <button
                                        class="w-full py-2 bg-white text-slate-950 text-xs uppercase font-bold hover:bg-slate-200">
                                        Rejoindre
                                    </button>
                                </form>
                            @elseif ($member->pivot->is_member == true)
                                <button disabled
                            ...
                            @else
                                <button disabled
                                    class="flex-1 py-2 bg-yellow-600 text-slate-950 text-xs uppercase font-bold cursor-not-allowed">
                                    En attente
                                </button>
                            @endif
                        @endauth
