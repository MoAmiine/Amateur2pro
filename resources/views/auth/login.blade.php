...
<body class="bg-slate-950 text-white min-h-screen flex items-center justify-center p-6">
    ...
    <div class="w-full max-w-sm">
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-6">
            @csrf
            @if ($errors->any())
                <div class="bg-red-500/10 border border-red-500/30 text-red-400 px-4 py-3 text-sm">
                    {{ $errors->first() }}
                </div>
            @endif
            <div>
                ...
                <input name="email" value="{{ old('email') }}" type="email"
                    class="w-full bg-slate-900 border border-white/10 p-4 rounded-sm focus:border-purple-500 outline-none transition-all">
